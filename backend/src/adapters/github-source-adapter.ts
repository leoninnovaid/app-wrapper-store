...
export class GitHubSourceAdapter implements SourceAdapter {
  ...
  async verifyArtifact(artifact: ReleaseArtifact): Promise<VerifyArtifactResult> {
    if (artifact.type === 'other') {
      return { status: 'blocked', reason: 'Artifact type is not installable' };
    }
    ...
    return { status: 'unverified', reason: 'No checksum metadata found on artifact' };
  }
  ...
  private mapArtifact(asset: GitHubAsset): ReleaseArtifact {
    ...
    const checksum = asset.digest?.trim();
    return {
      name: asset.name,
      type,
      platform,
      ...
      verificationStatus: checksum ? 'verified' : 'unverified',
