...
| G5-T3 | G5 | Expand regression coverage for core flows | In Progress | High | Core | G1-T4, G1-T5 | Backend lifecycle smoke landed; broader UI/e2e coverage still pending |
...
| Q3 | Add source/update UI screens in frontend | Done - per-app source validation, attachment, and update trust workflow landed |
| Q4 | Integrate signed Android artifact publishing | Required for production release |
| Q5 | Add e2e test suite for app -> source -> update -> build | In Progress - backend API plus wrapper-validation lifecycle smoke is landed; browser/frontend coverage remains |
...
| Q8 | Harden adapter trust parsing for edge cases (missing timestamps, malformed checksums) | Done - adapters now normalize invalid timestamps/checksums and update selection re-sorts releases deterministically |
...
|---|---|---|
| G5-T3 | Done when app creation, source validation, update-check, build trigger, and wrapper validation each have automated regression coverage with CI evidence. | Initial lifecycle smoke now exists in [backend/src/test/lifecycle.integration.test.ts](C:/Users/Leon/Documents/GitHub/app-wrapper-store/backend/src/test/lifecycle.integration.test.ts) and exercises wrapper validation via [wrapper-template/scripts/validate-wrapper.mjs](C:/Users/Leon/Documents/GitHub/app-wrapper-store/wrapper-template/scripts/validate-wrapper.mjs); next add browser/frontend-driven coverage guided by [docs/TESTING_DEBUGGING_ARCHITECTURE.md](C:/Users/Leon/Documents/GitHub/app-wrapper-store/docs/TESTING_DEBUGGING_ARCHITECTURE.md). |
...
| Q3 | Done when users can validate sources, attach them to apps, run update checks, and inspect trust state directly from the frontend. | Done - implemented per-app source/update workflow in [frontend/src/components/SourceUpdatePanel.tsx](C:/Users/Leon/Documents/GitHub/app-wrapper-store/frontend/src/components/SourceUpdatePanel.tsx), backed by state in [frontend/src/store/appStore.ts](C:/Users/Leon/Documents/GitHub/app-wrapper-store/frontend/src/store/appStore.ts) and API typing in [frontend/src/services/api.ts](C:/Users/Leon/Documents/GitHub/app-wrapper-store/frontend/src/services/api.ts). |
...
| Q5 | Done when the full `app -> source -> update -> build -> wrapper validation` path has automated end-to-end coverage with failure assertions. | Backend/wrapper smoke is now covered in [backend/src/test/lifecycle.integration.test.ts](C:/Users/Leon/Documents/GitHub/app-wrapper-store/backend/src/test/lifecycle.integration.test.ts); next expand into browser/frontend and generator-driven failure-path coverage using the staged model in [docs/TESTING_DEBUGGING_ARCHITECTURE.md](C:/Users/Leon/Documents/GitHub/app-wrapper-store/docs/TESTING_DEBUGGING_ARCHITECTURE.md). |
...
| Q8 | Done when trust signals from adapters are normalized into deterministic `verified`, `unverified`, or `blocked` outcomes with tests. | Done - added shared normalization in [backend/src/utils/source-normalization.ts](C:/Users/Leon/Documents/GitHub/app-wrapper-store/backend/src/utils/source-normalization.ts), applied it across [backend/src/adapters](C:/Users/Leon/Documents/GitHub/app-wrapper-store/backend/src/adapters), and verified deterministic selection/update behavior in [backend/src/test](C:/Users/Leon/Documents/GitHub/app-wrapper-store/backend/src/test). |
